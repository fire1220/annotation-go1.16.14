...
// 注释：译：gcAssistAlloc执行GC工作，使gp的援助债务为正。gp必须是调用用户gorountine。
//
...
func gcAssistAlloc(gp *g) {
	...
	// 注释：译：不要在不可抢占的情况下提供帮助。这些通常都很脆弱，不允许辅助阻挡。
	if getg() == gp.m.g0 {
		return
	}
...
retry:
	...
	// 注释：译：计算我们需要做的扫描工作量，以使平衡为正。当所需工作量较低时，我们会过度协助，为未来的分配建立信贷，并摊销协助成本。
