...
type mspan struct {
	...
	// 注释：(标记内存占用情况）8字节对齐的位图0未分配1已分配(uint8中每一位控制一个当前span的1块，会把其中64位补码放到缓存allocCache里，每种span的块数量固定【objects】位置：/src/runtime/sizeclasses.go)
	allocBits  *gcBits // 注释：块位图。当前span的所有块（每一位代表1块）
