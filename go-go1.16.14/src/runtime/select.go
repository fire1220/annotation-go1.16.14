...
func selectgo(cas0 *scase, order0 *uint16, pc0 *uintptr, nsends, nrecvs int, block bool) (int, bool) {
	...
	for i := range lockorder { // 注释：(遍历正常下标)程序向后遍历
		...
		lockorder[j] = pollorder[i] // 注释：把随机节点放到父节点上（如果父节点小于随机节点，则向上再次找父节点，以此类推，直到父节点大时停止，并赋值）
	}
	...
	for i := len(lockorder) - 1; i >= 0; i-- {
		...
		for {
			...
			if k >= i {  // 注释：(遍历到尾部退出)如果到最后一个子节点则退出循环(左子节点大于最后一个节点时退出)
				break
			}
			// 注释：比较左子节点和右子节点，记录大节点的key，k+1表示右子节点
			if k+1 < i && scases[lockorder[k]].c.sortkey() < scases[lockorder[k+1]].c.sortkey() {
				k++ // 注释：右子节点(k记录最大的节点下标)
			}
			// 注释：子节点和父节点比较，如果子节点大的话就和父节点交换数据，然后跳到子节点位置重新循环比较(这个顺序是从跟节点到子节点遍历)
			// 注释："尾节点"和"最大子节"点比较，如果尾节点小，(父节点后移)则父节点被子节点覆盖(上面父节点已经保存在尾节点位置了)，保证父节点最大，下载重新从当前父节点开始遍历
			if c.sortkey() < scases[lockorder[k]].c.sortkey() { // 注释：父节点和子节点比较
				lockorder[j] = lockorder[k] // 注释：(父节点后移)子节点赋值给父节点(k是最大子节点的下标，j是父节点下标)
				j = k                       // 注释：(节点下标后移到子节点上，下一次遍历时则为父节点)从子节点开始向后遍历
				continue
			}
