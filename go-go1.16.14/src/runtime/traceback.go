...
// 注释：译：通用追溯。处理运行时堆栈打印（ pcbuf == nil ），
// 		运行时。调用方函数（pcbuf!=nil），以及垃圾收集器（callback!=nile）。合并这些代码有点笨拙，但避免了重复代码及其所有微妙之处。
// 		skip参数仅对pcbuf!=nil时有效，并计算要跳过的逻辑帧数，而不是物理帧数（通过内联，pcbuf中的PC可以表示多个调用）。
//		如果部分跳过PC并且最大值>1，则pcbuf[1]将是运行时。
// 		skipPleaseUseCallersFrames+N，其中N表示pcbuf[0]中要跳过的逻辑帧数。
//
